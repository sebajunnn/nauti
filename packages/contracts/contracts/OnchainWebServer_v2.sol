...
contract OnchainWebServer_v2 {
    ...
    event PageUpdated(uint256 indexed pageId, address indexed owner);
    ...
    function updatePage(uint256 _pageId, string memory _newContent, string memory _contentType) external {
        require(msg.sender == pages[_pageId].owner, "Not the page owner");
        pages[_pageId].content = _newContent;
        pages[_pageId].contentType = _contentType;
        emit PageUpdated(_pageId, msg.sender);
    }
